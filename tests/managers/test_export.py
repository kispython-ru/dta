...
def test_export(db: AppDatabase):
    ...
    statuses = StatusManager(db.tasks, db.groups, db.variants, db.statuses, c, db.seeds, db.checks, ext)
    students = StudentManager(c, db.students, db.mailers)
    e = ExportManager(db.groups, db.messages, statuses, db.variants, db.tasks, db.students, students)
